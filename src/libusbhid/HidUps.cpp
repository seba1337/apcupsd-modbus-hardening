...
void HidUps::Close()
{
   ...
   // Grab the handle and clear _fd immediately, before tearing it down,
   // so a reentrant Close() call sees NULL right away rather than
   // attempting to tear down the same handle twice.
   usb_dev_handle *fd = _fd;
   _fd = NULL;
   ...
   if (fd)
   {
      ...
      // NOTE: we deliberately do NOT call usb_reset() here. libusb-0.1's
      // usb_reset() closes the underlying OS handle as part of resetting
      // the device (the device may re-enumerate, invalidating the fd),
      // so a usb_close() afterward -- as this function used to do -- is
      // a double-close of an already-torn-down handle. That's exactly
      // what crashed here: usb_release_interface() failing with EINVAL,
      // immediately followed by the (redundant) reset failing with
      // EBADF, then a NULL-pointer SIGSEGV inside libusb on the
      // subsequent usb_close(). init_device()'s own error_out cleanup
      // path (above) never resets either -- just release + close --
      // so this matches the pattern already used elsewhere in this file.
      usb_close(fd);
   }
}
