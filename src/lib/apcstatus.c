   ...
   int s_close(UPSINFO *ups, int sockfd))
{
   ...
   // LOADPCT is already "percent of rated real power" per the UPS's own
   // Modbus documentation (AN176) -- multiplying by the rated capacity
   // (NOMPOWER, already read from the UPS) gives live real power in
   // Watts without needing any new register or driver support.
   if (ups->UPS_Cap[CI_LOAD] && ups->UPS_Cap[CI_NOMPOWER])
      s_write(ups, "REALPOWER: %.1f Watts\n",
         ups->UPSLoad / 100.0 * ups->NomPower);

   if (ups->UPS_Cap[CI_LoadApparent])
      s_write(ups, "LOADAPNT : %.1f Percent\n", ups->LoadApparent);

   if (ups->UPS_Cap[CI_LoadApparent] && ups->UPS_Cap[CI_NomApparent])
      s_write(ups, "APPARENTPOWER: %.1f VA\n",
         ups->LoadApparent / 100.0 * ups->NomApparentPower);
